...
7. **Snapshot huérfano (jul-2026, el panel quedó congelado 6 días)**: el 7/7 un fallo TRANSITORIO de `SpreadsheetApp.openById(snapshotId)` dentro de `_getOrCreateSnapshotSS_` hizo que el builder CREARA un snapshot nuevo y repuntara el Script Property `INGECO_SNAPSHOT_ID`. El trigger siguió corriendo cada 30 min sin error… escribiendo en el snapshot nuevo, mientras el panel lee el ID viejo hardcodeado en `app.js` (`SNAPSHOT_ID = 1E883xvPP…`). Síntoma: datos viejos en el panel con META `__BUILD__` estancado, y un refresh que devuelve `ok:true` sin que cambie nada. Fix 13/7: repuntar el property al ID canónico + reintentos (3× con backoff) en `_getOrCreateSnapshotSS_` antes de recrear. Si vuelve a pasar: chequear `verSnapshotId()` vs el `SNAPSHOT_ID` de app.js. Quedó un snapshot huérfano `1t8LPOCMJumtP_1NUkjLxabcabIB0iZXWtGPvL8Dwxhg` en el Drive (borrable).

8. **Reestructura jul-2026 parte 3 — SERVICES + renumeración de flota**: la pestaña `OPERATIVIDAD` de SERVICES DE EQUIPOS se renombró a `RESUMEN` (mismo layout; el builder ya la busca por nombre y por header). Además la flota se RENUMERÓ en LISTA DE EQUIPOS (ej.: EXC-08→EXC-07 la Komatsu PC210, EXC-01→EXC-08 la CAT 320GX, RTP-03→RTP-11 la Hidromek …579, CMT-16..20→CMT-21..25) y los archivos operativos quedaron a medio migrar. El 13/7 se corrigieron por cruce patente/serie contra la LISTA: 151 filas de combustible livianos de AI-098-FR mal como CMT-21 (es CMT-17), 3 de AI-098-FS como CMT-22 (es CMT-26), servicios de AF-016-JM mal asignados (CMT-17→CMT-22; y el de 248304 km era de CMT-21/AD-426-PY), códigos basura en combustible pesados (Rtp08/Rtp05/Jtu107/Plt01). **Patrón**: ante código dudoso, la verdad es la patente/N° serie cruzada contra LISTA DE EQUIPOS.
